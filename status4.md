...
- Frontend visar:
  ...
  - tema-toggle (dark/light) med persistens i `localStorage`
  - auto-refresh (60s)
- Kan köras lokalt och nås från andra egna enheter via LAN/Tailscale.
- Har Docker/Compose-upplägg för frontend + backend.
...
- Frontend:
  - rubrik uppdaterad till `Marknadsöversikt`
  - sökruta borttagen från header
  - status flyttad till badges under tabbar
  - KPI-kort justerade till lägre höjd (~80% av tidigare)
  - tabell uppdaterad med `TID` (för råvaror), enhet i första kolumnen och tydlig `abs + %` i `+/-`
  - svenska talformat (decimal-komma/tusentalsmellanslag) är avgränsade bort och strukna som krav i aktuell spec-tolkning
- Drift:
...
- Test:
  ...
  - samlad testkörning via `./scripts/test-all.sh` (grön)
...
men flera krav från originalspecen (persistens, robust rate-limit-hantering och driftkrav) återstår.
