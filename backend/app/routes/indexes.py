...
def indexes_summary():
    ...
    instruments = [i for i in load_instruments() if i.module == "indexes"]
    ...
    cache.set(cache_key, items, fetched_at=fetched_at, update_last_update=has_fresh_values, module="indexes")
    global_stale = cache.is_globally_stale()
...
@router.get("/series")
def indexes_series(id: str, range: str = Query(default="1m", pattern="^(1m|3m|6m|1y)$")):
    cache_key = f"series:indexes:{id}:{range}"
    cached = cache.get(cache_key)
    if cached is not None:
        global_stale = cache.is_globally_stale()
        return {
    ...
    if instrument is None:
        raise HTTPException(status_code=404, detail=f"Unknown index id: {id}")

    points = fetch_series_for_instrument(instrument, range)
    fetched_at = datetime.now(timezone.utc)
    cache.set(cache_key, points, fetched_at=fetched_at, update_last_update=bool(points), module="indexes")
    global_stale = cache.is_globally_stale()
    return {
        "id": id,
        "range": range,
