...
@router.get("/series")
def mag7_series(id: str, range: str = Query(default="1m", pattern="^(1m|3m|6m|1y)$")):
    cache_key = f"series:mag7:{id}:{range}"
    cached = cache.get(cache_key)
    if cached is not None:
        global_stale = cache.is_globally_stale()
        return {
    ...
    if instrument is None:
        raise HTTPException(status_code=404, detail=f"Unknown Mag 7 id: {id}")

    points = fetch_series_for_instrument(instrument, range)
    fetched_at = datetime.now(timezone.utc)
    cache.set(cache_key, points, fetched_at=fetched_at, update_last_update=bool(points), module="mag7")
    global_stale = cache.is_globally_stale()
    return {
        "id": id,
        "range": range,
